...
import type { Roaster, Coffee, FlavorTag, TastingSession, TastingNote, RoasterListResponse, CoffeeListResponse, FlavorTagListResponse, TastingSessionListResponse, PaginatedResponse } from './types'
...
class ApiClient {
  ...
  // Roasters
  async getRoasters(skip: number = 0, limit: number = 100, search?: string): Promise<RoasterListResponse> {
  ...
  // Coffees
  async getCoffees(skip: number = 0, limit: number = 100, roasterId?: string): Promise<CoffeeListResponse> {
    const params = new URLSearchParams()
    params.append('skip', skip.toString())
    params.append('limit', limit.toString())
  ...
  async createCoffee(data: Partial<Coffee>): Promise<Coffee> {
  ...
  // Flavor Tags
  async getFlavorTags(skip: number = 0, limit: number = 100): Promise<FlavorTagListResponse> {
    const params = new URLSearchParams()
    params.append('skip', skip.toString())
    params.append('limit', limit.toString())
    
    return this.request<FlavorTagListResponse>(`/flavor-tags?${params.toString()}`)
  }
  ...
  // Tasting Sessions
  async getTastingSessions(skip: number = 0, limit: number = 100, coffeeId?: string): Promise<TastingSessionListResponse> {
    const params = new URLSearchParams()
    params.append('skip', skip.toString())
    params.append('limit', limit.toString())
    if (coffeeId) params.append('coffee_id', coffeeId)
    
    return this.request<TastingSessionListResponse>(`/tasting-sessions?${params.toString()}`)
  }
  ...
  async createTastingSession(data: Partial<TastingSession>): Promise<TastingSession> {
  ...
  async updateTastingSession(id: string, data: Partial<TastingSession>): Promise<TastingSession> {
    return this.request<TastingSession>(`/tasting-sessions/${id}`, {
      method: 'PUT',
      body: JSON.stringify(data),
    })
